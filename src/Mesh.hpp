...
#include <type_traits>
#include <vector>
#include </usr/local/Cellar/eigen/3.2.8/include/eigen3/Eigen/SparseCore>
#include </usr/local/Cellar/eigen/3.2.8/include/eigen3/Eigen/SparseCholesky>
...
class Mesh : public virtual NamedObject, private Noncopyable
{
  ...
  public:
    ...
    /** Constructor. Initialises priority queue (min heap), pq and map egde and index in pq.  */
...
// ------------------------------------------------------
	// --- Geodesic Heat Method, public mesh methods ---
    /** Get vertex for the given index */
    Vertex * getVertex(long index)
    {
    	assert(index < vertices.size());
  		long i = 0;
  		for (VertexIterator vi = verticesBegin(); vi != verticesEnd(); ++vi, i++)
     		if (i == index) return &(*vi);
     	return NULL;
	}

    /** Get an iterator pointing to the vertex for the given index */
    const Vertex * getVertexConst(long index) const
	{
		assert(index < vertices.size());
  		long i = 0;
  		for (VertexConstIterator vci = verticesBegin(); vci != verticesEnd(); ++vci, i++)
     		if (i == index) return &(*vci);
     	return NULL; 
	}
	
	/** Get the vertex index of the needlePoint */
	long getNeedlePointIndex() { return needlePointIndex; }
	
	/** Get the vertex index of the needlePoint */
	long getNeedlePointIndex() const { return needlePointIndex; }
	
	/** Compute Geodesics for Mesh */ 
    void computeGeodesics();
    
    /** Calculating the prefactors A - t*L  */
	void setup();
  ...
  // ------------------------------------------------------
	// --- Geodesic Heat Method, public mesh methods ---
	/** builds Cotan operator */
    void buildCotanOperator(Eigen::SparseMatrix<double>& L) const;
    
    /** builds one-third area operator */
    void buildAreaMatrix(Eigen::SparseMatrix<double>& A) const;
    
    /** computes time step */
    double computeTimeStep() const;
    
	/** Computes gradients for faces */
    void computeFaceGradients(Eigen::MatrixXd& gradients, const Eigen::VectorXd& u) const;
    
    /** Compute (an integrated) divergence (over neighbouring triangled faces) for a vertex */
	...
	// --- Geodesic Heat Method private variables ---
	long needlePointIndex;
