  ...
  if (mesh)
  {
    ...
    render_system->setMatrixMode(Graphics::RenderSystem::MatrixMode::PROJECTION); render_system->pushMatrix();
      ...
      render_system->pushShader();
        ...
        render_system->setColor(ColorRGB(0, 0.5, 0.5));
        mesh->draw(*render_system, /* draw_edges = */ show_edges, /* use_vertex_data = */ true, /* send_colors = */ false);
...
Viewer::keyPress(unsigned char key, int x, int y)
{
  ...
  else if (key == 'g' || key == 'G')
  {
    ...
    long npti = mesh->getNeedlePointIndex();
    highlighted_vertex = mesh->getVertex(npti);
    glutPostRedisplay();
  }
}
